...
export async function cropImageRegion(dataUrl: string, region: ImageRegion) {
  ...
  if (!context) {
    bitmap.close();
    throw new Error("Não foi possível recortar o balão.");
  }
...
export async function extractVerticalGlyphs(
  dataUrl: string,
  region: ImageRegion,
): Promise<VerticalGlyph[]> {
  const blob = await (await fetch(dataUrl)).blob();
  const bitmap = await createImageBitmap(blob);
  const left = Math.max(0, Math.round(region.x)),
    top = Math.max(0, Math.round(region.y));
  ...
  for (let i = 0, p = 0; i < dark.length; i++, p += 4)
    dark[i] =
      pixels[p] < 145 && pixels[p + 1] < 145 && pixels[p + 2] < 145 ? 1 : 0;
  const ranges = (counts: number[], minimum: number, gap: number) => {
    ...
    if (start >= 0) found.push([start, last]);
    return found;
  };
  const xCounts = Array.from({ length: width }, (_, x) => {
    let count = 0;
    for (let y = 0; y < height; y++) count += dark[y * width + x];
    return count;
  });
  const candidateColumns = ranges(
    xCounts,
    2,
    Math.max(2, Math.round(width * 0.008)),
  ).filter(([a, b]) => b - a >= 4 && b - a < width * 0.45);
  const widestColumn = Math.max(
    0,
    ...candidateColumns.map(([a, b]) => b - a + 1),
  );
  ...
  const columns = candidateColumns
    .filter(([a, b]) => b - a + 1 >= widestColumn * 0.52)
    .sort((a, b) => b[0] - a[0]);
  const glyphs: VerticalGlyph[] = [];
  for (const [x0, x1] of columns) {
    const columnWidth = x1 - x0 + 1;
    const yCounts = Array.from({ length: height }, (_, y) => {
      let count = 0;
      for (let x = x0; x <= x1; x++) count += dark[y * width + x];
      return count;
    });
    const rows = ranges(
      yCounts,
      1,
      Math.max(1, Math.round(columnWidth * 0.07)),
    ).filter(([a, b]) => b - a >= 3);
    for (const [y0, y1] of rows) {
      let ink = 0;
      for (let y = y0; y <= y1; y++)
        for (let x = x0; x <= x1; x++) ink += dark[y * width + x];
      if (ink < 12) continue;
      const glyphWidth = x1 - x0 + 1,
        glyphHeight = y1 - y0 + 1,
        size = Math.max(glyphWidth, glyphHeight) + 24;
      const glyph = new OffscreenCanvas(size, size),
        glyphContext = glyph.getContext("2d");
      ...
      glyphs.push({
        image: await canvasDataUrl(glyph),
        bbox: {
          x: region.x + x0,
          y: region.y + y0,
          width: glyphWidth,
...
export async function detectSpeechBubbles(
  ...
  for (let index = 0, p = 0; index < white.length; index++, p += 4) {
    ...
    white[index] =
      pixels[p] >= 238 && pixels[p + 1] >= 238 && pixels[p + 2] >= 238 ? 1 : 0;
  }
  ...
  for (let seed = 0; seed < white.length; seed++) {
    ...
    while (head < queue.length) {
      ...
      for (const next of neighbors) {
        if (next < 0 || next >= white.length || visited[next] || !white[next])
          continue;
        const nx = next % width;
    ...
    if (
      ...
      continue;
    const pad = Math.max(2, Math.round(Math.min(boxWidth, boxHeight) * 0.035));
    regions.push({
      x: (minX + pad) / scale,
      y: (minY + pad) / scale,
      width: (boxWidth - pad * 2) / scale,
      height: (boxHeight - pad * 2) / scale,
    });
  }
  return regions
